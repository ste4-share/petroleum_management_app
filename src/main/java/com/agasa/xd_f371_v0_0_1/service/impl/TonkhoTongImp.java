...
public class TonkhoTongImp implements TonkhoTongService {
    ...
    @Override
    public TonkhoTong findById(int tonkhotongId) {
        QDatabase.getConnectionDB();
        String SQL_SELECT = "Select * from tonkho_tong where id=?";
        try {
            PreparedStatement preparedStatement = QDatabase.conn.prepareStatement(SQL_SELECT);
            preparedStatement.setInt(1, tonkhotongId);
            ResultSet resultSet = preparedStatement.executeQuery();
    ...
    @Override
    public TonkhoTong findByQuarterAndXdAll(int quarter_id1, int xd_id1) {
        QDatabase.getConnectionDB();
    ...
    public TonkhoTong create(TonkhoTong tonkhoTong) {
        QDatabase.getConnectionDB();
        String sql = "insert into tonkho_tong(id_quarter, id_xd, amount, tck_nvdx, tck_sscd, tck_sum,tdk_nvdx, tdk_sscd, tdk_sum) " +
                "values(?,?,?,?,?,?,?,?,?)";
        try {
            ...
            statement.setInt(7, tonkhoTong.getTdk_nvdx());
            statement.setInt(8, tonkhoTong.getTdk_sscd());
            statement.setInt(9, tonkhoTong.getTdk_sum());
    ...
    public TonkhoTong update(TonkhoTong tonKho) {
        QDatabase.getConnectionDB();
        String sql = "update tonkho_tong set tck_sum=?,tck_nvdx=?,tck_sscd=?,tdk_sum=?,tdk_nvdx=?,tdk_sscd=? where id_quarter=? and id_xd=?";
        try {
            PreparedStatement statement = QDatabase.conn.prepareStatement(sql);
            statement.setInt(1, tonKho.getTck_sum());
            statement.setInt(2, tonKho.getTck_nvdx());
            statement.setInt(3, tonKho.getTck_sscd());
            statement.setInt(4, tonKho.getTdk_sum());
            statement.setInt(5, tonKho.getTdk_nvdx());
            statement.setInt(6, tonKho.getTdk_sscd());
            statement.setInt(7, tonKho.getId_quarter());
            statement.setInt(8, tonKho.getId_xd());
            statement.executeUpdate();
        } catch (SQLException e) {
            e.printStackTrace();
            throw new RuntimeException(e);
